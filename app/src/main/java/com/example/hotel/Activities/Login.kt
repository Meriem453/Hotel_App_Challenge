        ...
        button.setOnClickListener {
            ...
            lifecycleScope.launch {
                ...
                //decode the token
                    ...
                    editor.putString("name","${response.body()?.user?.nom} ${response.body()?.user?.prenom}")
                    editor.apply()

                    editor.putString("role",response.body()?.user?.status)
                    editor.apply()
Toast.makeText(baseContext,"Don't forget to async",Toast.LENGTH_SHORT).show()
